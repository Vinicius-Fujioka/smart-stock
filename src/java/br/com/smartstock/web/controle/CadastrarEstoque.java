    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
                try{
                    ...
                    Integer refereidloja = Integer.parseInt(request.getParameter("idloja"));

                    Estoque estoque = new Estoque();
                    estoque.setNome(nome);
                    estoque.setDescricao(descricao);
                    estoque.setRefereidloja(refereidloja);
        ...
        if(idestoque.equals("")){
                ...
                if(dao.alterar(estoque) == true){
                    mensagem = "Estoque alterado com sucesso!";
                }else{
                    mensagem = "Erro ao alterar estoque!";
                }
                
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("gerenciarestoques-loja.jsp").forward(request, response);
            
        }catch(Exception e){
            System.out.println("Erro ao cadastrar estoqueCTR " + e.getMessage());
        }
    }
